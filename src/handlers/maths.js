...
export const randomMathOperation = (numOfDigits1, numOfDigits2, forcedOperation, firstNumber) => {
  const num1 = firstNumber || getRandomNumberDigits(numOfDigits1, false);
  ...
  switch (operation) {
    ...
    case 1:
      ...
      if (num2 > num1) {
        if (firstNumber) {
          return randomMathOperation(numOfDigits1, numOfDigits2, null, firstNumber);
        } else {
          returnObj.op1 = num2;
          returnObj.op2 = num1;
    ...
    case 3:
      ...
      if (num1 >= num2 && (num1 % num2) === 0) {
        returnObj.result =  num1 / num2;
      } else if (!firstNumber && (num2 > num1 && (num2 % num1) === 0)) {
        returnObj.result =  num2 / num1;
      } else if (firstNumber) {
        return randomMathOperation(numOfDigits1, numOfDigits2, null, firstNumber);
      } else {
        return randomMathOperation(numOfDigits1, numOfDigits2, 3, firstNumber);
      }
      break;
    default:
      break;
  }
  // Para controlar el modo cadena
  if (firstNumber) {
    const resultLength = returnObj.result.toString().length;
    if (resultLength > (numOfDigits1+1) || resultLength > (numOfDigits2+1)) {
      return randomMathOperation(numOfDigits1, numOfDigits2, null, firstNumber);
    }
  }
